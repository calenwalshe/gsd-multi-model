...
Based on `TARGET_MODE` from SKILL.md, resolve the list of phases to drive.
...
When `/gsd:drive` needs context for a phase, generate a minimal CONTEXT.md directly instead of running discuss-phase interactively. This avoids user prompts during autonomous driving.
...
Resume: Run /gsd:drive to continue from this point.
```
...
3. Check if more phases remain in `PHASES` list:
   ...
   - If no: proceed to final summary (Step 5 in SKILL.md)
...
1. Clear the auto-chain flag:
   ...
   ```
2. Return to SKILL.md Step 5 for final summary output
